  ...
  <div class="flex flex-col items-center gap-6 mt-4">
    ...
    <div class="flex items-start gap-6">
      <div class="bg-[#1e1e1e] rounded-lg border border-gray-700 overflow-hidden shadow-xl">
        ...
        ><span class="text-[#569cd6]">template</span> <span class="text-[#d4d4d4]">&lt;</span><span class="text-[#569cd6]">typename</span> <span class="text-[#4ec9b0]">T</span><span class="text-[#d4d4d4]">,</span> <span class="text-[#569cd6]">unsigned</span> <span class="text-[#9cdcfe]">N</span><span class="text-[#d4d4d4]">&gt;</span>
...
<span class="text-[#d4d4d4]">};</span></pre>
    ...
    <div class="flex items-start gap-8 mt-4">
      ...
      <div class="flex flex-col items-center">
        ...
        <div
          class="flex border-2 rounded overflow-hidden transition-all duration-500"
          :class="isOverflowed ? 'border-gray-600' : 'border-green-400'"
        >
          <div
            v-for="i in inlineCapacity"
            :key="'inline-' + i"
            class="font-mono text-center border-r last:border-r-0 transition-all duration-500"
            :class="[
              isOverflowed
          ...
          >
            {{ !isOverflowed && i <= elements.length ? elements[i - 1] : '·' }}
          </div>
        </div>
        <div
          class="text-xs mt-2 transition-all duration-500"
      ...
      <!-- Arrow -->
      <div
        class="flex flex-col items-center justify-center h-20 transition-all duration-500"
      ...
      <div class="flex flex-col items-center">
        ...
        >
          <div
            ...
            :class="[
              isOverflowed
                ? 'bg-yellow-800 text-white border-yellow-400'
                : 'bg-gray-700 text-gray-500 border-gray-600',
              isOverflowed && i <= elements.length ? 'opacity-100' : 'opacity-30',
            ]"
            :style="{ width: cellWidth + 'px', padding: '12px 0' }"
    ...
    <!-- Status message -->
    <div
      class="mt-4 px-4 py-2 rounded-lg transition-all duration-500"
      :class="
        isOverflowed
