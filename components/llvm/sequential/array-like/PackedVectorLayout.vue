...
// Animation sequence
const animations = [
    { values: [0, 0, 0, 0, 0, 0, 0, 0], highlighted: null, message: "PackedVector<unsigned, 2> — 8 elements × 2 bits = 16 bits total" },
    ...
    { values: [3, 1, 0, 2, 0, 0, 0, 1], highlighted: null, message: "All values packed into just 16 bits!" },
]
    ...
    <div class="flex flex-col items-center gap-4 mt-4">
        ...
        <div class="flex items-start gap-6">
            <div class="bg-[#1e1e1e] rounded-lg border border-gray-700 overflow-hidden shadow-xl">
                <pre
                    class="px-4 py-3 font-mono text-sm leading-relaxed"><span class="text-[#569cd6]">template</span> <span class="text-[#d4d4d4]">&lt;</span><span class="text-[#569cd6]">typename</span> <span class="text-[#4ec9b0]">T</span><span class="text-[#d4d4d4]">,</span> <span class="text-[#569cd6]">unsigned</span> <span class="text-[#9cdcfe]">BitNum</span><span class="text-[#d4d4d4]">&gt;</span>
...
<span class="text-[#d4d4d4]">};</span> <span class="text-[#6a9955]">// Each element uses exactly BitNum bits!</span></pre>
        ...
        <div class="flex flex-col items-center">
            <div class="text-gray-400 text-sm mb-2 font-semibold">Logical View ({{ numElements }} elements)</div>
            <div class="flex gap-1">
                <div v-for="(val, idx) in values" :key="'elem-' + idx" class="flex flex-col items-center">
                    ...
                    <div class="w-10 h-10 flex items-center justify-center font-mono text-lg border-2 rounded transition-all duration-300"
                        :class="[
        ...
        <div class="flex flex-col items-center">
            <div class="text-gray-400 text-sm mb-2 font-semibold">Physical Storage (BitVector — {{ totalBits }} bits)
            </div>
            <div class="flex">
                <div v-for="(val, idx) in values" :key="'bits-' + idx"
                    class="flex border-r-2 last:border-r-0 transition-all duration-300" :class="[
                        highlightedElement === idx
                            ? 'border-orange-400'
                            : 'border-gray-600'
                    ]">
                    <div v-for="(bit, bitIdx) in getBitsForElement(idx).split('')" :key="'bit-' + idx + '-' + bitIdx"
                        class="w-6 h-8 flex items-center justify-center font-mono text-sm transition-all duration-300"
                        :class="[
                            highlightedElement === idx
                                ? (bit === '1' ? 'bg-orange-500 text-white' : 'bg-orange-800 text-orange-200')
                                : (bit === '1' ? 'bg-green-700 text-white' : 'bg-gray-800 text-gray-500')
                        ]">
                        {{ bit }}
                    </div>
                </div>
            ...
            <div class="flex mt-1">
                <div v-for="idx in numElements" :key="'label-' + idx"
