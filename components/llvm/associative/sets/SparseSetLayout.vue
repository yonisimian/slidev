...
<template>
  <div class="flex flex-col items-center justify-center">
    <!-- Title -->
    <div class="text-lg font-bold text-pink-400 mb-4">SparseSet - Two-Array O(1) Set</div>
    ...
    <div class="mt-4 flex gap-2">
      <div
        v-for="(label, i) in ['Empty', 'Insert 3,7', 'Insert 1', 'Erase 7']"
        :key="i"
        class="px-2 py-1 rounded text-xs transition-all duration-300"
        :class="phase === i ? 'bg-pink-500 text-white' : 'bg-gray-700 text-gray-400'"
      >
        {{ label }}
      </div>
    </div>
    ...
    >
      Lookup: sparse[v] → if valid, dense[sparse[v]] == v → O(1) | Delete: swap last into hole →
      O(1)
    </div>
  </div>
</template>
