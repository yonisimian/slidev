  ...
  <div class="flex flex-col items-center justify-center">
    ...
    <div class="flex gap-12 items-start">
      ...
      <div class="flex flex-col items-center">
        ...
        <div class="flex flex-col gap-1">
          <div
            v-for="(str, i) in strings.slice(0, phase)"
            :key="str"
    ...
    <div class="mt-8 flex gap-3">
      <div
        v-for="(label, i) in ['Empty', 'insert if', 'insert else', 'insert while']"
        :key="i"
        class="px-3 py-1 rounded text-xs transition-all duration-300"
        :class="phase === i ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-400'"
      >
        {{ label }}
      </div>
    </div>
